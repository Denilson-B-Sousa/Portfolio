...
import { Airplay, GithubLogo, X, ArrowSquareOut, Globe } from "phosphor-react";
  ...
  return (
    <Dialog.Root>
      <DialogTrigger className="rounded-md flex justify-center flex-col md:flex-row-2 h-80 w-80 md:w-64 md:h-72 text-left p-4 md:p-4 gap-2 bg-light-500 dark:bg-gray-600  hover:ring-2 hover:ring-purple-800 focus-visible:ring-2 focus-visible:ring-purple-800 outline-none firefox-card">
        ...
        <h4 className="text-xl font-semibold dark:text-primary text-black">{title}</h4>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="flex flex-col fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[90vh] md:rounded-md md:bg-gray-600 my-8 outline-none">
          ...
          <ScrollArea.Root className="flex justify-center items-center m-auto w-[100vw] md:w-[85%] p-4 rounded overflow-hidden">
            <ScrollArea.Viewport className="w-[80vw] h-[62vh] md:h-[80vh] rounded bg-light-500 dark:bg-gray-600 p-6">
              <img src={imageURI} alt="" width={600} className="rounded-lg" />
              <div className="dark:text-primary text-black">
                <div className="flex justify-between mt-2">
                  <span>
                    <Airplay width={32} height={32} />
                  </span>
                </div>
              ...
              <ScrollArea.Scrollbar
                ...
                className="hidden md:flex justify-end select-none touch-none p-0.5 bg-gray-600 transition-colors duration-[160ms] ease-out data-[orientation=vertical]:w-2 data-[orientation=horizontal]:flex-col data-[orientation=horizontal]:h-[100%]"
              >
                <ScrollArea.Thumb className="flex-1 bg-mauve10 relative before:content-[''] before:absolute before:top-1/2 before:left-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:w-full before:h-full before:min-w-[44px] before:min-h-[44px] bg-purple-900 rounded-full" />
              </ScrollArea.Scrollbar>
